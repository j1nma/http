...
START_TEST(test_get_eq)
{
    const struct parser_definition d = parser_utils_strcmpi("GET ");

    struct parser *parser = parser_init(parser_no_classes(), &d);
    assert_eq(STRING_CMP_MAYEQ, 'G', parser_feed(parser, 'G'));
    assert_eq(STRING_CMP_MAYEQ, 'E', parser_feed(parser, 'E'));
    assert_eq(STRING_CMP_MAYEQ, 'T', parser_feed(parser, 'T'));
