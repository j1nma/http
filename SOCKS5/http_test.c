...
static void
on_hello_method(struct http_parser *p, const uint8_t method) {
...
START_TEST (test_hello_unsupported_version) {
    ...
    bool errored = false;
    enum parser_state st = parser_consume(&b, &parser, &errored);
    ck_assert_uint_eq(parser_error_unsupported_version, st);
...
START_TEST (test_hello_multiple_request) {
    ...
    st = parser_consume(&b, &parser, &errored);
    ck_assert_uint_eq(false, errored);
